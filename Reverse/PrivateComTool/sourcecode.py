...
class des():
    ...
    def run(self, key, text, action=ENCRYPT, padding=False):
        if len(key) < 8:
            raise "yjuykjhgrfrgthyjukilykjthrgfergthyukiloiykthr5grfvgy5g"
        elif len(key) > 8:
            key = key[:8] 
        ...
        if padding and action==ENCRYPT:
            ...
            raise "dfghggfdvdfewrt354thrbgdfvdert454yhrbgdffet35y4htrbdf"
...
if __name__ == '__main__':
    key = int(raw_input('Provide the Agency Secret Key: '))
    if key >= 9999999:
        key = str(key)
        text = raw_input('Provide the Agency Secret Message: ')
        d = des()
        r = d.encrypt(key,text)
        with open('Message.Agency', 'wb') as f:
            f.write(r)
    else:
        print ("They key dose not respect the Agency policy! Try again!")
